package com.kraos.swipeexpandlistview.ListView;
...
/**
 * Description: 折叠&左滑listview
 * Author:Kraos dengbch@crop.21cn.com
 * Date:2019/9/10 10:42
 */
public class SwipeExpandableListView extends ExpandableListView {
    private float mLastMotionX; //记录x坐标
    private float mLastMotionY; //记录y坐标w
    private SwipeItemLayout mCaptureItem;
    private VelocityTracker mVelocityTracker;
    private int mActivePointerId;
    private int mTouchSlop;
    private int mMaximumVelocity;
    private boolean isDragHandleSuper; //是否交给父类处理的拖拽
    private boolean isDragHandleThis; //是否交给自身处理的拖拽
    private boolean isCancelEvent; //如果item open，此时点击其它地方就close item，并且完全不处理后续的所以消息（即此时滑动不起作用了）。
    ...
    private void init(Context context) {
        ...
        mActivePointerId = -1;
        isDragHandleSuper = false;
        isDragHandleThis = false;
        isCancelEvent = false;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if(isCancelEvent && action!=MotionEvent.ACTION_UP && action!=MotionEvent.ACTION_CANCEL)
            return true;
        ...
        switch (action){
            case MotionEvent.ACTION_DOWN:{
                ...
                if(!isDragHandleThis)
                    isDragHandleSuper = super.onInterceptTouchEvent(ev);
                return isDragHandleThis || isDragHandleSuper;
            }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if(isDragHandleSuper && mCaptureItem!=null)
                    mCaptureItem.close();
                return isDragHandleThis || isDragHandleSuper;
            }
            ...
            case MotionEvent.ACTION_UP:
                boolean ret = false;
                if(isDragHandleThis && mCaptureItem!=null/**起始一定不为null*/){
                    SwipeItemLayout.Mode touchMode = mCaptureItem.getTouchMode();
                    if(touchMode== SwipeItemLayout.Mode.DRAG){
                ...
                if(mCaptureItem!=null){
                    ...
                    if(touchMode== SwipeItemLayout.Mode.TAP ){
                        ...
                        if(xDiff>mTouchSlop && xDiff>yDiff){
                            ...
                            isDragHandleSuper = true;//super.onInterceptTouchEvent(ev);
                    ...
                    isDragHandleSuper = super.onTouchEvent(ev);
